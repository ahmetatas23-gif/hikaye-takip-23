...
ON CONFLICT (tc_no) DO NOTHING;
        ...
        function closeResultModalIfOpen() {
            ...
            if (window._resultCountdownTimer) {
                clearInterval(window._resultCountdownTimer);
                window._resultCountdownTimer = null;
            }
        }
        ...
        async function submitCevapKagidi() {
            ...
            // showTestResult'ın soru detay bölümü için currentTest.answers'ı doldur
            currentTest.answers = {};
            ...
            try {
                ...
                if (existing) {
                    showNotification('⚠️ Bu testi zaten tamamladınız!', 'warning');
                    return;
                }
                ...
                const { error } = await supabase.from('test_results').insert({
                    ...
                    answers: JSON.stringify({
                        ...
                        soruAnaliz,           // toggleOptikFormTable ve renderOptikSoruAnaliz için
                ...
                if (testTimer) clearInterval(testTimer);
                await awardBadges(score, Math.floor((new Date() - currentTest.startTime) / 1000));

                // Önce ekranı gizle, sonuç modalını aç (normal test akışıyla aynı)
                hideAllScreens();
                document.getElementById('studentDashboard').classList.remove('hidden');
                showTestResult(score, correctAnswers, total, earnedPoints, totalPoints);

                // Arkaplanda dashboard datasını yenile (modal açıkken sessizce)
                loadStudentData().catch(e => console.warn('Background reload error:', e));

            } catch (err) {
                console.error('submitCevapKagidi error:', err);
                showNotification('Gönderilirken hata oluştu: ' + err.message, 'error');
            }
        }
        ...
        async function submitTest() {
            ...
            currentTest.questions.forEach((question, index) => {
                ...
                if (question.type === 'multiple_choice') {
                    ...
                    let correctAnswerBool = question.correct;
                    if (typeof question.correct === 'string') {
                        correctAnswerBool = question.correct === 'true' || question.correct === true;
                    }
        ...
        function showTestResult(score, correct, total, earnedPoints = null, totalPoints = null) {
                ...
                <div class="text-center">
                    <div class="mb-4">
                        ...
                        <p class="text-sm text-gray-600 mb-1">
                            ${passed ? 'Başarı puanına (%70+) ulaştınız!' : 'Başarı puanına ulaşamadınız.'}
                        </p>
                    ...
                    <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4 mb-4 border border-blue-100">
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
                            ...
                            <div class="bg-white rounded-lg p-3 shadow-sm">
                                ...
                                <div class="text-xs text-gray-500">
                                    ${total - correct === 0 ? '🎯 Mükemmel!' : '📚 Çalış'}
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-2 max-h-32 overflow-y-auto">
                            ${currentTest.questions.map((question, index) => {
                                ...
                                if (currentTest._isCevapKagidi) {
                                    // Cevap kağıdı: harflerle göster (seçenek sayısına göre)
                                    ...
                                    userLabel = userOpt || '-';
                                    correctLabel = revMap[question.correct] || '?';
                                ...
                                const typeLabel = currentTest._isCevapKagidi ? 'Cevap Kağıdı' :
                                    question.type === 'multiple_choice' ? 'Çoktan Seçmeli' :
                                    question.type === 'fill_blank' ? 'Boşluk Doldurma' : 'Doğru/Yanlış';
                                ...
                                return `
                                    <div class="flex justify-between items-center py-2 px-3 rounded ${isCorrect ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}">
                                        <div class="flex items-center space-x-2">
                                            ...
                                            ${currentTest._isCevapKagidi ? `
                                                <span class="text-xs text-gray-500">Cevabın: <b>${userLabel}</b> / Doğru: <b>${correctLabel}</b></span>
                                            ` : `
                                                <span class="text-xs text-gray-600">(${typeLabel})</span>
                                            `}
                                        </div>
                    ...
                    ${!passed ? `
                        <div class="bg-red-50 border border-red-200 rounded-lg p-3 mb-4">
                            <div class="flex items-center mb-2">
                                ...
                                <h4 class="text-sm font-semibold text-red-800">Önemli Uyarı</h4>
                            </div>
                            <p class="text-red-800 text-sm font-medium mb-1">
                                Öğretmeninizin tekrar atamasını bekleyin.
                            </p>
                            <p class="text-red-700 text-xs">
                                Tekrar çözebilmek için öğretmeninizin yeniden atama yapması gerekir.
                            </p>
                        </div>
                    ` : `
                        <div class="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
                            <div class="flex items-center mb-2">
                                ...
                                <h4 class="text-sm font-semibold text-green-800">Harika İş!</h4>
                            </div>
                            <p class="text-green-800 text-sm">
                                Başarıyla geçtiniz! Yeni rozetler kazanmış olabilirsiniz.
                            </p>
                        </div>
                    `}
                    ...
                    <div class="text-xs text-gray-500">
                        ...
                        Otomatik kapanma: <span id="resultCountdownText">120</span> saniye
                    </div>
                </div>
            `;
            ...
            // Geri sayım
            let remaining = 120;
            window._resultCountdownTimer = setInterval(() => {
                remaining--;
                const cd1 = document.getElementById('resultCountdown');
                const cd2 = document.getElementById('resultCountdownText');
                if (cd1) cd1.textContent = `(${remaining})`;
                if (cd2) cd2.textContent = remaining;
                if (remaining <= 0) {
